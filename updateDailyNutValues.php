...
// Help function
function calc_perc($arrNutValues, $nut_name) {
    $curDRIGoal = floatval($_SESSION['dailyNutritionGoalsDict'][$nut_name]);
    //if ($curDRIGoal > 0):
    $percOfDRI = floatval($arrNutValues[$nut_name])/$curDRIGoal*100;
...
// Adding total values to the table meta-data
...
for ($ii = 0; $ii < count($arrNutValues); $ii++) {
    ...
    $percOfDRI = calc_perc($arrNutValues, $arrColsNames[$ii]);
